...
function initTable(){
	...
	$tb.bootstrapTable({
		...
		url: projectName+"/manage/questions/search",         //后台的服务器地址
		...
		queryParams:function(params){   //自定义查询参数
			...
			params["subjectId"]=$("#search_subjectId").val();
		...
		columns:[
			{radio:true},
			{field:"id",title:"试题编号"},
			{field:"subjectId",title:"试题类型",formatter:subjectFormatter},
			{field:"questionName",title:"试题题目"},
			{field:"typeId",title:"试题类型",formatter:typeFormatter},
...
function operateFormatter(value, row, index){
	var url=projectName+"/manage/questions/edit?id="+row.id;
...
function subjectFormatter(value, row, index){
	return row.subjectArea.subjectName;
}
 
//格式化 单选还是多选
function typeFormatter(value, row, index){
	return value==1?"<span class='label label-primary'>单选题</span>":"<span class='label label-success'>多选题</span>";
}
